...
# Para la búsqueda en profundidad (LIFO)

def Stack():
    """Return an empty list, suitable as a Last-In-First-Out Queue."""
    return []

#Para la búsqueda en anchura (FIFO)
...
def pathCost(element):
    return element.path_cost

# Para la búsqueda de ramificación y acotación
...
class SortedListByCost(Queue):
    ...
    # Añade por el final => x=[1,2,3] | x.append([4,5]) | print(x) = [1,2,3,[4,5]]
    ...
    # Añade por el final y ordena por el coste de menor a mayor => x=[1,2,3] | x.append([4,5]) | print(x) = [1,2,3,4,5]

    def extend(self, items):
        self.A.extend(items)
        self.A.sort(key=pathCost, reverse=True)
...
# Para la búsqueda de ramificación y acotación con subestimación

class SortedListByHeuristicAndCost(Queue):
    """Sorted list by path cost of the element."""
    ...
    def heuristicAndCost(self, element):
        return pathCost(element) + self.problem.h(element)
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A.sort(key=self.heuristicAndCost, reverse=True)
